...
from Child.forms import UsForm,ComplaintForm,Newuserform,ChpwdForm,Donateform
from django.core.mail import send_mail
from Children_Welfare import settings
from django.contrib import messages
from django.contrib.auth.decorators import login_required
from Child.models import User,Donate
...
def occdetails(request):
	if request.method=="POST":
		...
		if occdetails.is_valid():
			l=occdetails.save(commit=False)
			l.uid_id=request.user.id
			l.save()
			return redirect('/show')
...
def userinfo(ty,uname):
	p=Donateform.objects.get(User_Name=uname)
	h=Donateform.objects.get(pid_id=p.id)
