...
from Child.forms import UsForm,ComplaintForm,Newuserform,ChpwdForm,Donateform,Occdonateform,Onetimedonform#,Serviceform
from django.core.mail import send_mail
from Children_Welfare import settings
from django.contrib import messages
from django.contrib.auth.decorators import login_required
from Child.models import User,Donate,Occdonate,Onetimedon#,service
...
def donatedetails(request):
	if request.method=="POST":
		details=Donateform(request.POST)
		if details.is_valid():
...
def occdetails(request):
	if request.method=="POST":
		occdetails=Occdonateform(request.POST)
		if details.is_valid():
			k=details.save(commit=False)
			k.uid_id=request.user.id
			k.save()
			return redirect('/show')
	occdetails=Occdonateform()
	return render(request,'html/occdetails.html',{'occdetails':occdetails})

def onetimedetails(request):
	if request.method=="POST":
		details=Onetimedonform(request.POST)
		if details.is_valid():
			k=details.save(commit=False)
			k.uid_id=request.user.id
			k.save()
			return redirect('/show')
	onedetails=Onetimedonform()
	return render(request,'html/onetimedetails.html',{'onedetails':onedetails})
...
def showinfo(request):
	data=Donate.objects.filter(uid_id=request.user.id)
	return render(request,'html/showdata.html',{'info1':data})


def deletedata(req,st):
	data=Donate.objects.get(id=st)
	#print(data)
	if req.method=="POST":
		data.delete()
		return redirect('/show')
...
def userupdate(up,si):
	t=Donate.objects.get(id=si)
	if up.method=="POST":
		d=Donateform(up.POST,instance=t)
		if(d.is_valid()):
			d.save()
			return redirect('/show')
	d=Donateform(instance=t)
...
# 			return redirect('/show')
# 	ser=Serviceform()
# 	return render(request,'html/service.html',{'ser':ser})
